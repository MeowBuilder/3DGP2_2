...
void CMirrorShader::RenderReflectedObjects(ID3D12GraphicsCommandList* pd3dCommandList, CCamera* pCamera)
{
	...
	{
		m_pScene->m_pSkyBox->Render(pd3dCommandList, &reflectedCamera, 1); // 사용자 제안: xmmtxReflect 인자 추가
	}
	
	// (2) Terrain : 마찬가지로 월드는 그대로, 카메라만 반사
	if (m_pScene->m_pTerrain)
	{
		m_pScene->m_pTerrain->Render(pd3dCommandList, &reflectedCamera, xmmtxReflect, 1); // 사용자 제안: xmmtxReflect 인자 추가
	}
