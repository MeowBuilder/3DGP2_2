...
CMirrorShader::CMirrorShader(CScene* pScene, CGameObject* pMirrorObject)
{
	...
	pMirrorMaterial->m_xmf4AmbientColor = XMFLOAT4(0.0f, 0.0f, 1.0f, 1.f);
...
void CMirrorShader::CreatePipelineStates(ID3D12Device* pd3dDevice, ID3D12RootSignature* pd3dGraphicsRootSignature)
{
	...
	ds0.DepthWriteMask = D3D12_DEPTH_WRITE_MASK_ZERO;
	...
	ds0.FrontFace.StencilPassOp = D3D12_STENCIL_OP_REPLACE;
	...
	bs0.RenderTarget[0].RenderTargetWriteMask = 0;
	...
	ds2.DepthWriteMask = D3D12_DEPTH_WRITE_MASK_ZERO;			
...
void CMirrorShader::RenderReflectedObjects(ID3D12GraphicsCommandList* pd3dCommandList, CCamera* pCamera)
{
	...
	XMVECTOR xmvPoint = XMVectorSet(m_pMirrorObject->m_xmf4x4World._41,m_pMirrorObject->m_xmf4x4World._42,m_pMirrorObject->m_xmf4x4World._43, 1.0f);
	...
	{
		m_pScene->m_pTerrain->Render(pd3dCommandList, &reflectedCamera, xmmtxReflect, 1); 
	}
